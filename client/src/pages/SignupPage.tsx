...
import { Rocket, PartyPopper, Hourglass, GraduationCap, MailCheck, KeyRound, Mail, AlertTriangle, ArrowLeft, ShieldCheck, Lock } from 'lucide-react';
 ...
 *   2. Identity       — college email + name + username, or Continue with
 *                       Google (auto-verifies on domain match, skips ahead).
 ...
 *
 * BACK vs BYPASS: going back is free while NO ACCOUNT EXISTS. Once signup
 * created it, the college is locked server-side (updateProfile 403s) and the
 * email can only move forward through a fresh OTP — there is no path to the
 * password step (or the app) that skips verification, client or server.
 */

type Phase = 'college' | 'identity' | 'otp' | 'password';
const STEPS: Phase[] = ['college', 'identity', 'otp', 'password'];
...
export default function SignupPage() {
 ...
 try {
   ...
   await verificationApi.sendCollegeEmail(email);
   startCooldown();
   setOtp('');
   setPhase('otp');
 ...
 const handleSetPassword = async () => {
   ...
   try {
     ...
     sessionStorage.removeItem('signup:phase');
 ...
 const backToCollege = () => {
   // Before submit this is a free redo (no account exists yet). After submit
   // the server now LOCKS the college — this link only rewinds the wizard.
   setStepError(null);
   setPhase('college');
 };
 ...
 {/* Progress */}
 <div className="flex gap-1 mb-2" role="progressbar" aria-valuemin={1} aria-valuemax={4} aria-valuenow={stepIndex + 1} aria-label={`Step ${stepIndex + 1} of 4`}>
 {[0, 1, 2, 3].map((s) => (
 <div
 key={s}
 aria-hidden="true"
 className={`h-1.5 flex-1 border-nb-2 border-ink ${
 s <= stepIndex ? 'bg-nb-violet' : 'bg-gray-200'
 }`}
 />
 ))}
 </div>
 ...
 />
 <p className="text-xs text-gray-500 mt-1">Your college is your world here — everything you see stays inside it. Double-check the pick: it locks once your account is created.</p>
 {college && (
 domain ? (
 ...
 {phase === 'identity' && (
 <div className="space-y-4 animate-slide-up">
 <button type="button" onClick={backToCollege} className="text-sm opacity-60 hover:opacity-100 inline-flex items-center gap-1">
 <ArrowLeft size={14} /> {college?.name || 'College'} <span className="opacity-60">— change?</span>
 </button>
 <div>
 ...
 {stepError && (
 <p role="alert" className="text-sm text-nb-pink font-semibold flex items-center justify-center gap-1">
 ...
 <button onClick={() => { setStepError(null); setPhase('identity'); }} className="text-sm opacity-60 hover:opacity-100">
 <ArrowLeft size={13} className="inline mr-1" />Wrong email or details? Edit them — a new code goes to the new address.
 </button>
