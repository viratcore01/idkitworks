...
async function main() {
  ...
  // Idempotent guard: re-running the seed must never duplicate posts (author+content is the identity).
  const ensurePost = (args: { data: Prisma.PostCreateArgs['data'] }) =>
    prisma.post
      .findFirst({ where: { authorId: args.data.authorId as string, content: args.data.content as string } })
      .then((found) => found ?? prisma.post.create(args));
  ...
  // Comments: only seed when none exist on the seeded posts (createMany is not idempotent)
  const seededPostIds = posts.map((p) => p.id);
  const existingComments = await prisma.comment.count({ where: { postId: { in: seededPostIds } } });
  if (existingComments === 0) {
    ...
    data: [
      ...
      { postId: posts[6].id, authorId: ishita.id, content: "I'll bring the coffee, you bring the notes" },      { postId: posts[8].id, authorId: sneha.id, content: "DM sent! Himachal looks gorgeous" },
      ],
    });
  }
