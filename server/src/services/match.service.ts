...
export class MatchService {
  ...
  async discover(userId: string, page = 0, limit = DECK_PAGE_SIZE) {
    ...
    const result = {
      users: pageSlice.map((u: any) => ({
        ...
        avatarPhotoId: u.avatarPhotoId ?? null,
  ...
  async getMatches(userId: string, page = 0, limit = 50) {
    ...
    const matches = await prisma.match.findMany({
      ...
      include: {
        userAObj: { select: { id: true, username: true, displayName: true, avatarUrl: true, avatarPhotoId: true, bio: true } },
        userBObj: { select: { id: true, username: true, displayName: true, avatarUrl: true, avatarPhotoId: true, bio: true } },
      },
      orderBy: { createdAt: 'desc' },
      skip: page * take,
      take: take + 1,
